...
// check if a given file is a .pptx (Office Open XML presentation)
static bool IsPptxArchive(MultiFormatArchive* archive) {
    ByteSlice ct = archive->GetFileDataByName("[Content_Types].xml");
    if (!ct) {
        return false;
    }
    bool res = str::Find((char*)ct.data(), "presentationml") != nullptr;
    ct.Free();
    return res;
}
...
Kind GuessFileTypeFromContent(const char* path) {
    ...
    if (res == kindFileZip) {
        ...
        if (archive) {
            if (IsPptxArchive(archive)) {
                res = kindFilePptx;
            } else if (IsXpsArchive(archive)) {
                res = kindFileXps;
            } else if (IsEpubArchive(archive)) {
                res = kindFileEpub;
            } else if (IsFb2Archive(archive)) {
                res = kindFileFb2z;
            }
            delete archive;
        }
